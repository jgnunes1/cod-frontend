import { obtemDetalhe, obtem, salva, edita, apagaApi } from "../../api/Fakes/FakeApi";
import { useEffect, useState } from "react";
import { FakeTelefone } from '../../api/Fakes/services/FakeTelefone';

export const FakeDetalheContext = (url = "", schema = "repo") => {
  ...
  const obterDetalhe = (id_mestre = 0) => {

    const jsonFake = fakeJson(url);

    const json = obtemDetalhe(id_mestre, rota_base, jsonFake);
  ...
  return {
    ...
    salvar: item => requestHandler(salva, item, false, obterDetalhe,),
    editar: item => requestHandler(edita, item, true, obterDetalhe,),
    apagar: item => requestHandler(apagaApi, item, false, obterDetalhe),
    setField,
  };
};
